...
# ============================================================
# DARK THEME CSS  (replaces original light CSS)
# ============================================================
st.markdown("""
<style>
    ...
    [data-testid="stSidebar"] label p { font-size: 15px; font-weight: 500; color: #C0C0E0; }
    [data-testid="stSidebar"] .stRadio label { font-size: 15px; color: #C0C0E0; }
    [data-testid="stSidebar"] p,
    [data-testid="stSidebar"] span,
    [data-testid="stSidebar"] div { color: #C0C0E0; }
    ...
    .rec-card-text { color: #C0C0E0; font-size: 0.9rem; line-height: 1.7; }
    .rec-threshold {
        border-radius: 8px;
        padding: 8px 14px;
        font-size: 0.88rem;
    ...
    /* ── Selectbox / Radio ── */
    .stSelectbox label, .stRadio label { color: #C0C0E0 !important; }
    ...
    /* set via fig.patch in each chart below */
</style>
...
# ============================================================
# DATA PATHS  (unchanged from original)
# ============================================================
...
section = st.sidebar.radio(
    "Section",
    ["Overview", "Descriptive", "Diagnostic", "Prescriptive", "Predictive", "Recommendations"]
)

# ── Sidebar labels (full original questions — unchanged) ──
...
subpages = {
    "Descriptive":      ["KPIs & Gender Distribution", Q_DESC_1, Q_DESC_2, Q_DESC_3, Q_DESC_4, Q_DESC_5],
...
# ── helper: which tab to show based on sidebar section ──
TAB_MAP = {
    "Overview":        tab1,
    "Descriptive":     tab2,
    "Diagnostic":      tab3,
    "Prescriptive":    tab4,
    "Predictive":      tab5,
    "Recommendations": tab6,
}
active_tab = TAB_MAP[section]
...
- **Recommendations** — Consolidated actionable interventions and final conclusion
        ...
        st.info("👈 Select **Overview** in the sidebar to view this section.")
...
with tab2:
    if section != "Descriptive":
        st.info("👈 Select **Descriptive** in the sidebar to view this section.")
    else:
        # ── KPIs & Gender Distribution ──────────────────────────────────────────
        if page == "KPIs & Gender Distribution":
        ...
        # ── Q2 Trends ───────────────────────────────────────────────────────────
            ...
            ax.axhline(180, color="#FF4C4C",  linestyle="--", alpha=0.8,
                       label="Hyperglycemia (180)")
            ax.axhline(70,  color="#FF9F1C",  linestyle="--", alpha=0.8,
            ...
            ax.tick_params(axis="both", labelsize=12, colors="#C0C0E0")
            ax.legend(fontsize=12, facecolor="#1A1A2E", edgecolor="#3A3A5A",
                      labelcolor="#C0C0E0")
...
with tab3:
    if section != "Diagnostic":
        st.info("👈 Select **Diagnostic** in the sidebar to view this section.")
    else:
        # ── Q1 Sleep vs Glucose ─────────────────────────────────────────────────
        if page == Q_DIAG_1:
        ...
        # ── Q3 Steps vs Variability ─────────────────────────────────────────────
            ...
            axes[0].set_title("Prev Day Steps vs Glucose Standard Deviation",
                              fontsize=12, color="#E0E0FF")
            axes[0].tick_params(colors="#C0C0E0")
            ...
            sns.regplot(data=daily, x="Prev_Day_Steps", y="Daily_Glucose_CV",
                        ...
                        line_kws={"color": "#FF4C4C"}, ax=axes[1])
            axes[1].set_title("Prev Day Steps vs Glucose CV (%)",
                              fontsize=12, color="#E0E0FF")
            axes[1].tick_params(colors="#C0C0E0")
...
# ============================================================
with tab4:
    if section != "Prescriptive":
        st.info("👈 Select **Prescriptive** in the sidebar to view this section.")
    else:
...
with tab5:
    if section != "Predictive":
        st.info("👈 Select **Predictive** in the sidebar to view this section.")
    else:
        if page == Q_PRED_1:
...
with tab6:
    ...
    recs = [
        ...
        {
            "icon": "🥗",
            "title": "Dietary Intervention for High-Risk Patients",
            "color": "#FF4C4C",
            "bg":    "#2A1A1A",
            "finding": (
        ...
        {
            "icon": "🚶",
            "title": "Daily Step Target for Glucose Stability",
            "color": "#2ECC71",
            "bg":    "#1A2A1A",
            "finding": (
        ...
        {
            "icon": "❤️",
            "title": "Dual HR + Glucose Spike Response",
            "color": "#FF9F1C",
            "bg":    "#2A1A0A",
            "finding": (
        ...
        {
            "icon": "💉",
            "title": "Insulin Timing & Bolus Optimisation",
            "color": "#00D4AA",
            "bg":    "#0A1A1A",
            "finding": (
    ...
    # ── Summary Matrix ─────────────────────────────────────────────────────────
    st.markdown("<hr style='border-color:#2A2A4A; margin:1.5rem 0;'>",
                unsafe_allow_html=True)
